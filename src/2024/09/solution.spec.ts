...
describe('day 01', () => {
  describe('part 01', () => {
    describe.each([['solution', part1Solution]])('%s', (_, solution) => {
      test('example input', () => {
        expect(solution(exampleInput)).toBe(1928);
      });

      test('real input', () => {
        expect(solution(actualInput)).toBe(6283170117911);
      });
    });
  });
  ...
  describe('part 02', () => {
    describe.each([['solution', part2Solution]])('%s', (_, solution) => {
      test('example input', () => {
        expect(solution(exampleInput)).toBe(2858);
      });

      test('real input', () => {
        expect(solution(actualInput)).toBe(6307653242596);
      });
    });
  });
});
